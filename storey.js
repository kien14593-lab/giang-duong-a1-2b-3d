...
function facade(B, g, i, z0, z1) {
  ...
  if (i === 8) {
    const tex = canvasTexture(256, 256, (c) => {
      c.fillStyle = '#fff'; c.beginPath(); c.arc(128, 128, 126, 0, 7); c.fill();
      c.strokeStyle = '#1f5fa8'; c.lineWidth = 12; c.beginPath(); c.arc(128, 128, 108, 0, 7); c.stroke();
      c.fillStyle = '#1f5fa8'; c.textAlign = 'center'; c.textBaseline = 'middle';
      c.font = 'bold 70px Arial'; c.fillText('UTC', 128, 112); c.font = 'bold 30px Arial'; c.fillText('GTVT', 128, 170);
    });
    const disc = new THREE.Mesh(new THREE.CylinderGeometry(1.1, 1.1, 0.06, 48), new THREE.MeshStandardMaterial({ map: tex, roughness: 0.5, clippingPlanes: planes }));
